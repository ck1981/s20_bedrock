...
Vagrant.configure('2') do |config|
  ...
  if Vagrant::Util::Platform.windows?
    ...
    hosts_path = File.join(ANSIBLE_PATH, 'hosts/development')
    #config_path = File.join(ANSIBLE_PATH, 'ansible.cfg')        #test
    #ansible_roles_path = File.join(ANSIBLE_PATH, 'vendor', 'roles', '/')     #test
    
    config.vm.provision :shell do |sh|
      sh.path = 'windows.sh'
      sh.args = "#{playbook_path} #{hosts_path}"                           
      #sh.args = "#{playbook_path} #{hosts_path} #{ansible_roles_path}"         #test
    end
  else
